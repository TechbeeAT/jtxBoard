...
class MainActivity2 : AppCompatActivity() {
    ...
    companion object {
        ...
        const val INTENT_EXTRA_CATEGORY2PRESELECT = "category2preselect"
        ...
        if (intent.hashCode() != lastProcessedIntentHash) {
            when (intent?.action) {
                INTENT_ACTION_ADD_JOURNAL -> {
                    ...
                    globalStateHolder.icalFromIntentCategory.value = intent.getStringExtra(INTENT_EXTRA_CATEGORY2PRESELECT)
                    intent.removeExtra(INTENT_EXTRA_CATEGORY2PRESELECT)
                }
